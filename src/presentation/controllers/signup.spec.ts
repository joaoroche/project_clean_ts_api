...
import { AccountModel } from '../../domain/models/accont'
...
describe('SignUp Controller', () => {
  ...
  test('Shoulds return 400 if no passwordConfirmation is provided', () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'any_name',
  ...
  test('Shoulds return 400 if no password confirmation fails', () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'any_name',
  ...
  test('Shoulds call AddAcount with correct values', () => {
